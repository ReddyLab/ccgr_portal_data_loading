...
class SDMetadata(ScreenMetadata):
    @property
...
def line_to_data(line: list[str]) -> EncodeData:
    ...
    log10pValue = float(log10pValue)
    log10pValueAdjusted = float(log10pValueAdjusted)
    significance_threshold = log10(0.02)
    significant = log10pValueAdjusted >= significance_threshold
